...
function StaggerClient(zmq, registrationAddress) {
  ...
  this.register(registrationAddress || "tcp://127.0.0.1:5867");
}
...
prototype.registerCallback = function(callback) {
  this.callbacks.push([callback, new StaggerAggregator(this.zmqClient)]);
};
...
prototype.runAndReportCallbacks = function(timestamp) {
  async.eachLimit(this.callbacks, 10, function(item, next) {
    ...
    new StaggerAggregator(this.zmqClient).report(timestamp, {
      complete: true,
    });
  }.bind(this));
};
